...
export function sessionUserName(user): string {
    if (!user) {
        return consts.Account.ModeNames[consts.Account.Fierce];
    } else if (user.code !== consts.Account.Conspirator) {
        return user.name;
    } else {
        return consts.Account.ModeNames[consts.Account.Conspirator];
    }
}
...
export function listUserName(name: string, id: number): string {
    if (name) {
        return name;
    } else if (id === consts.Account.Id.Fierce) {
        return consts.Account.ModeNames[consts.Account.Fierce]
    } else {
        return consts.Account.ModeNames[consts.Account.Conspirator];
    }
...
export function createToken(id: number, password: string): string | number[] {
