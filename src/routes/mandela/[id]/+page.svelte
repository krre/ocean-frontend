<script lang="ts">
	...
	let totalVotes = $derived(votes ? votes.reduce((sum, v) => sum + v.count, 0) : 0);
		...
		{#if !$userSession.isAnonym && ($userSession.id === mandela.user_id || $userSession.isAdmin)}
			<div class="buttons">
				...
				{#if $userSession.isAdmin}
					...
					{#if mandela.trash}
						<button onclick={() => updateTrash(false)}>Переместить в каталог</button>
					{:else}
						<button onclick={() => updateTrash(true)}>Переместить в хлам</button>
					{/if}
				{/if}
			</div>
		{/if}
	...
	<div class="container">
		{#if $userSession.isAnonym || (vote != null && !editVote)}
			...
			{#if !$userSession.isAnonym}
				<div class="buttons">
					<button onclick={() => (editVote = true)}>Изменить выбор</button>

					{#if $userSession.isAdmin}
						<button onclick={getVoteUsers}>{voteUserVisible ? 'Скрыть' : 'Показать'}</button>
					{/if}
				</div>
		...
		{:else}
			...
			<div>
				<WaitButton title="Проголосовать" enabled={voteValue >= 0} sendAction={castVote} />
			</div>
		{/if}
	</div>
</Rectangle>
